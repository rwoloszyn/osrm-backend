                                                              ...
                                                              const Intersection &intersection)
{
    ...
    std::cout << "Checking for Reverse (" << leftmost_tag << ")" << std::endl;
                                              ...
                                              Intersection intersection) const
{
    ...
    {
        std::cout << "In: " << data.lane_string_id << " out: "
                  << node_based_graph.GetEdgeData(intersection[1].turn.eid).lane_string_id
                  << std::endl;
        std::cout << "Nothing to do, suppressed turn and turn lanes continue" << std::endl;
        return intersection;
    }
        ...
        lane_data.size() + 1 == possible_entries)
    {
        ...
        std::cout << "Invalidated U-Turn at Location: " << std::setprecision(12)
                  << toFloating(node_info_list[at].lat) << " " << toFloating(node_info_list[at].lon)
                  << std::endl;
        intersection[0].entry_allowed = false;
    }

    if (!lane_data.empty() && detail::canMatchTrivially(intersection, lane_data) &&
        lane_data.size() !=
                                           ...
                                           const Intersection &intersection) const
{
    ...
    {
        std::cout << "NUm Turns and none doesn't match" << std::endl;
        std::cout << "Num Turns: " << num_turns << " lane size: " << lane_data.size() << std::endl;
        return false;
    }
    ...
    for (const auto &data : lane_data)
    {
        ...
        const auto best_match = [&]() {
            if (data.tag != "reverse" || lane_data.size() == 1)
                return detail::findBestMatch(data.tag, intersection);

            // lane_data.size() > 1
            if (lane_data.back().tag == "reverse")
                return detail::findBestMatchForReverse(lane_data[lane_data.size() - 2].tag,
                                                       intersection);

            BOOST_ASSERT(lane_data.front().tag == "reverse");
            return detail::findBestMatchForReverse(lane_data[1].tag, intersection);
        }();
        ...
        std::cout << "Tag: " << data.tag << " matched to: " << match_index
                                   ...
                                   const Intersection &intersection) const
{
    ...
    for (std::size_t lane = 0; lane < turn_lane_data.size(); ++lane)
    {
        ...
        const auto best_match = detail::findBestMatch(turn_lane_data[lane].tag, intersection);
        if (detail::isValidMatch(turn_lane_data[lane].tag, best_match->turn.instruction))
        {
        ...
        const auto best_match_at_next_intersection =
            detail::findBestMatch(turn_lane_data[lane].tag, next_intersection);
        if (detail::isValidMatch(turn_lane_data[lane].tag,
    ...
    std::cout << "Straightmost Tag: " << straightmost_tag_index << " of " << turn_lane_data.size()
              << std::endl;
    ...
    std::cout << "[First]\n";
    for (auto lane : first)
        std::cout << "\t" << lane.tag << " " << (int)lane.from << " " << (int)lane.to << std::endl;
    std::cout << "[Second]\n";
    for (auto lane : second)
        std::cout << "\t" << lane.tag << " " << (int)lane.from << " " << (int)lane.to << std::endl;
                                                       ...
                                                       const LaneDataVector &lane_data) const
{
    ...
    for( auto entry : lane_data )
        std::cout << entry.tag << " " << (int)entry.from << " " << (int)entry.to << std::endl;
    /*
    if (!(lane_data.size() ==
          static_cast<std::size_t>(
              (lane_data.back().tag == "reverse" && !intersection[0].entry_allowed) ? 1 : 0) +
    ...
    */
    ...
    if (valid_turn < lane_data.size())
    {
        ...
        if (lane_data.back().tag == "reverse" && valid_turn + 1 == lane_data.size())
        {
            ...
            std::cout << "Adding U-Turn for reverse lane" << std::endl;
